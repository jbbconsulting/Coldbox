...
<!----------------------------------------- CONSTRUCTOR ------------------------------------->
	...
	<!--- init --->
	<cffunction name="init" access="public" returntype="Builder" hint="Constructor. If called without a configuration binder, then WireBox will instantiate the default configuration binder found in: cachebox.system.ioc.config.DefaultBinder" output="false" >
		<cfargument name="injector" type="any" required="true" hint="The linked WireBox injector" colddoc:generic="cachebox.system.ioc.Injector"/>
		<cfscript>
			instance = {
			...
			if( instance.injector.isColdBoxLinked() ){
				instance.coldboxDSL = createObject("component","cachebox.system.ioc.dsl.ColdBoxDSL").init( arguments.injector );
			}
			// Is CacheBox Linked?
			if( instance.injector.isCacheBoxLinked() ){
				instance.cacheBoxDSL = createObject("component","cachebox.system.ioc.dsl.CacheBoxDSL").init( arguments.injector );
			}
			// Build LogBox DSL Namespace
			instance.logBoxDSL = createObject("component","cachebox.system.ioc.dsl.LogBoxDSL").init( arguments.injector );
...
<!------------------------------------------- DSL BUILDER METHODS ------------------------------------------>
	...
	<cffunction name="getProviderDSL" access="private" returntype="any" hint="Get dependencies using the our provider pattern DSL" output="false" >
		...
		<cfscript>
			...
			return createObject("component","cachebox.system.ioc.Provider").init( argumentCollection=args );
		</cfscript>
	</cffunction>
