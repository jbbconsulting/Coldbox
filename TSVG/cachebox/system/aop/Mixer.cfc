...
component accessors="true"{
	...
	*/
	function configure( required injector, required properties ){
		...
		// mixer util
		variables.mixerUtil 			= new cachebox.system.aop.MixerUtil();
		// class id code
		...
		if( NOT structKeyExists( variables.properties, "generationPath" ) ){
			variables.properties.generationPath = "/cachebox/system/aop/tmp";
		}
	...
	private function weaveAdvice(
		...
		>
			<cfscript>
				...
				var invocation = createObject( "component", "cachebox.system.aop.MethodInvocation" ).init(
