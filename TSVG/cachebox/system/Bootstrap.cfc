...
component serializable="false" accessors="true"{
	...
	*/
	function loadColdBox(){
		...
		application[ appKey ] = new cachebox.system.web.Controller( COLDBOX_APP_ROOT_PATH, appKey );
	...
	*/
	private string function processException( required controller, required exception ){
		...
		var oException	= new cachebox.system.web.context.ExceptionBean( arguments.exception );
		...
		if( len( customErrorTemplate ) ){
			...
			savecontent variable="local.exceptionReport"{
				include "/cachebox/system/includes/BugReport-Public.cfm";
			}
		}
