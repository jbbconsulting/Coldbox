...
component extends="cachebox.system.testing.BaseTestCase"{
	...
	*/
	function setup(){
		...
		// Create Mock Objects
		variables.mockController 	 		= mockBox.createMock( "cachebox.system.testing.mock.web.MockController" );
		variables.mockInterceptorService 	= mockbox.createEmptyMock( "cachebox.system.web.services.InterceptorService" );
		variables.mockRequestContext 		= getMockRequestContext();
		variables.mockRequestService 		= mockBox.createEmptyMock( "cachebox.system.web.services.RequestService" ).$( "getContext", variables.mockRequestContext);
		...
		variables.mockFlash		 	 		= mockBox.createMock( "cachebox.system.web.flash.MockFlash" ).init(mockController);
		variables.mockCacheBox   	 		= mockBox.createEmptyMock( "cachebox.system.cache.CacheFactory" );
		variables.mockWireBox		 		= mockBox.createEmptyMock( "cachebox.system.ioc.Injector" );
