...
component serializable="false" accessors="true"{
	...
	*/
	function init( required appRootPath, appKey="cbController" ){
		// Create Utility
		variables.util 			= new cachebox.system.core.util.Util();
		// services scope
		variables.services 		= createObject( "java", "java.util.LinkedHashMap" ).init( 6 );
		// CFML Engine Utility
		variables.CFMLEngine 	= new cachebox.system.core.util.CFMLEngine();
		...
		services.loaderService 		= new cachebox.system.web.services.LoaderService( this );
		variables.logBox 			= services.loaderService.createDefaultLogBox();
		variables.log 				= variables.logBox.getLogger( this );

		// Setup the ColdBox Services
		services.requestService 	= new cachebox.system.web.services.RequestService( this );
		services.handlerService 	= new cachebox.system.web.services.HandlerService( this );
		services.moduleService 		= new cachebox.system.web.services.ModuleService( this );
		services.interceptorService = new cachebox.system.web.services.InterceptorService( this );
		
		// CacheBox Instance
		variables.cacheBox 	= createObject("component","cachebox.system.cache.CacheFactory");
		// WireBox Instance
		variables.wireBox	= createObject("component","cachebox.system.ioc.Injector");
	...
	* 
	* @return cachebox.system.cache.IColdboxApplicationCache
	*/
...
/****************************************** PRIVATE HELPERS *************************************************/
	...
	*/
	private function loadColdBoxSettings(){
		...
		structAppend( variables.coldboxSettings, new cachebox.system.web.config.Settings(), true );
	}
