...
01/18/2007 - Created
----------------------------------------------------------------------->
<cfcomponent name="RequestService" output="false" hint="This service takes care of preparing and creating request contexts. Facades to FORM and URL" extends="cachebox.system.web.services.BaseService">
...
<!------------------------------------------- PUBLIC ------------------------------------------->
	...
	<!--- Request Capture --->
	<cffunction name="requestCapture" access="public" returntype="any" output="false" hint="I capture an incoming request. Returns: cachebox.system.web.context.RequestContext" colddoc:generic="cachebox.system.web.context.RequestContext">
		<cfscript>
	...
	<cffunction name="eventCachingTest" access="public" output="false" returntype="void" hint="Tests if the incoming context is an event cache">
		<cfargument name="context" 	required="true"  type="any" hint="The request context to test for event caching." colddoc:generic="cachebox.system.web.context.RequestContext">
	...
	<!--- getFlashScope --->
    <cffunction name="getFlashScope" output="false" access="public" returntype="any" hint="Get the current running Flash Ram Scope of base type:cachebox.system.web.flash.AbstractFlashScope">
    ...
    <cffunction name="buildFlashScope" output="false" access="public" returntype="any" hint="Build's the Flash RAM Scope as defined in the application spec.">
   		<cfscript>
			...
			switch( instance.flashData.scope ){
				case "session" : {
					flashpath = "cachebox.system.web.flash.SessionFlash";
					break;
				}
				case "client" : {
					flashpath = "cachebox.system.web.flash.ClientFlash";
					break;
				}
				case "cluster" : {
					flashpath = "cachebox.system.web.flash.ClusterFlash";
					break;
				}
				case "cache" : {
					flashpath = "cachebox.system.web.flash.ColdboxCacheFlash";
					break;
				}
				case "mock" : {
					flashpath = "cachebox.system.web.flash.MockFlash";
					break;
				}
				default : {
...
<!------------------------------------------- PRIVATE ------------------------------------------->
	...
	<cffunction name="createContext" access="private" output="false" returntype="any" hint="Creates a new request context object">
		...
		oContext = CreateObject("component","cachebox.system.web.context.RequestContext").init( properties=controller.getConfigSettings(), controller=controller );
