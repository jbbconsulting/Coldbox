...
*/
component extends="cachebox.system.web.services.BaseService"{
	...
	*/
	function createDefaultLogBox(){
		...
		logBoxConfig = new cachebox.system.logging.config.LogBoxConfig( CFCConfigPath="cachebox.system.web.config.LogBox" );

		return new cachebox.system.logging.LogBox( logBoxConfig, controller );
	}
	...
	*/
	LoaderService function createWireBox(){
		...
		binder.map( "HTMLHelper@coldbox" )
			.to( "cachebox.system.core.dynamic.HTMLHelper" );
		// Map Renderer
		binder.map( "Renderer@coldbox" )
			.to( "cachebox.system.web.Renderer" );
		// Map Data Marshaller
		binder.map( "DataMarshaller@coldbox" )
			.to( "cachebox.system.core.conversion.DataMarshaller" );
		// Map XML Converter
		binder.map( "XMLConverter@coldbox" )
			.to( "cachebox.system.core.conversion.XMLConverter" );
		// Map Object Converter
		binder.map( "ObjectMarshaller@coldbox" )
			.to( "cachebox.system.core.conversion.ObjectMarshaller" );
	...
	*/
	LoaderService function createCacheBox(){
		var config 				= createObject("Component","cachebox.system.cache.config.CacheBoxConfig").init();
	...
	*/
	function createAppLoader( overrideConfigFile="" ){
		...
		return new cachebox.system.web.config.ApplicationLoader( controller );
	}
