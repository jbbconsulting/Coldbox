...
----------------------------------------------------------------------->
<cfcomponent extends="cachebox.system.web.services.BaseService" hint="This is the main Coldbox Handler service" output="false">
...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->
	...
	<cffunction name="init" access="public" output="false" returntype="HandlerService" hint="Constructor">
		...
		<cfscript>
			...
			// Handler base class
			instance.HANDLER_BASE_CLASS = "cachebox.system.EventHandler";
...
<!------------------------------------------- PUBLIC ------------------------------------------->
		...
		<cfscript>
			...
			if( NOT wirebox.getBinder().mappingExists( invocationPath ) ){
				...
				mapping = wirebox.registerNewInstance( name=invocationPath, instancePath=invocationPath )
					.setVirtualInheritance( "cachebox.system.EventHandler" )
	...
	<cffunction name="getHandler" output="false" access="public" returntype="any" hint="Returns a valid event handler object ready for execution">
		<!--- ************************************************************* --->
		<cfargument name="ehBean" 			type="any" required="true" hint="The event handler bean to use: cachebox.system.web.context.EventHandlerBean"/>
	...
	<cffunction name="getRegisteredHandler" access="public" hint="I parse the incoming event string into an event handler bean that is used for executions." returntype="cachebox.system.web.context.EventHandlerBean"  output="false">
		<!--- ************************************************************* --->
		...
		var handlersList 			= instance.registeredHandlers;
		var handlersExternalList 	= instance.registeredExternalHandlers;
		var handlerBean 			= CreateObject("component","cachebox.system.web.context.EventHandlerBean").init(instance.handlersInvocationPath);
		var moduleReceived			= "";
	...
	<cffunction name="invalidEvent" output="false" access="private" returntype="void" hint="Invalid Event procedures. Throws EventHandlerNotRegisteredException">
		...
		<cfargument name="ehBean" type="any" 	required="true" hint="The event handler bean" colddoc:generic="cachebox.system.web.context.EventHandlerBean"/>
		<cfscript>
			var iData			= structnew();
