...
----------------------------------------------------------------------->
<cfcomponent output="false" hint="The coldbox interceptor service" extends="cachebox.system.web.services.BaseService">
...
<!------------------------------------------- CONSTRUCTOR ------------------------------------------->
	...
	<cffunction name="init" access="public" output="false" returntype="InterceptorService" hint="Constructor">
		...
		<cfscript>
			...
			instance.requestBuffer = CreateObject("component","cachebox.system.core.util.RequestBuffer").init();
			// Default Logging
			instance.log = controller.getLogBox().getLogger( this );
    		// Setup Default Configuration
    		instance.interceptorConfig = {};
			// Interceptor base class
			instance.INTERCEPTOR_BASE_CLASS = "cachebox.system.Interceptor";
...
<!------------------------------------------- PUBLIC ------------------------------------------->
		...
		*/
		public any function processState( 
			...
			numeric asyncJoinTimeout=0 
				...
				var requestBuffer = new cachebox.system.core.util.RequestBuffer();
		...
		<cfscript>
			...
			if( NOT wirebox.getBinder().mappingExists( "interceptor-" & interceptorName ) ){
				...
				wirebox.registerNewInstance(name="interceptor-" & interceptorName, instancePath=interceptorClass)
					...
					.setThreadSafe( true )
					.setVirtualInheritance( "cachebox.system.Interceptor" )
	...
	<cffunction name="getRequestBuffer" access="public" returntype="any" output="false" hint="Get a coldbox request buffer: cachebox.system.core.util.RequestBuffer">
	...
	<cffunction name="registerInterceptionPoint" access="public" returntype="any" hint="Register an Interception point into a new or created interception state." output="false" >
		...
		<cfscript>
			...
			if ( NOT structKeyExists( instance.interceptionStates, arguments.state ) ){
				oInterceptorState = new cachebox.system.web.context.InterceptorState( 
	...
	<cffunction name="parseMetadata" returntype="struct" access="private" output="false" hint="I get a components valid interception points">
		...
		<cfscript>
			...
			if ( structKeyExists( arguments.metadata, "extends" ) and
				 (arguments.metadata.extends.name neq "cachebox.system.Interceptor" and
				  arguments.metadata.extends.name neq "cachebox.system.EventHandler" )
			){
				// Recursive lookup
